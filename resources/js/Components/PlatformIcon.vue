<template>
    <div :class="classes" :title="platformName">
        <!-- PlayStation -->
        <svg v-if="isPlayStation" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 16" fill="currentColor" class="w-full h-full">
        ...
        <!-- Xbox -->
        <svg v-else-if="isXbox" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-full h-full">
        ...
        <!-- Nintendo (Switch) -->
        <svg v-else-if="isNintendo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 16" fill="currentColor" class="w-full h-full">
            <path fill-rule="evenodd" d="M8 0h5a8 8 0 110 16H8A8 8 0 118 0zm-.135 1.935a6.065 6.065 0 000 12.13h5.12a6.065 6.065 0 000-12.13h-5.12zm-1.33 2.304h2.401l3.199 5.175V4.24h2.346v7.495H12.18L8.864 6.537v5.201H6.53l.005-7.499z"/>
        </svg>

        <!-- Mac -->
        <svg v-else-if="isMac" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 18" fill="currentColor" class="w-full h-full">
        ...
        <!-- Linux -->
        <svg v-else-if="isLinux" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 18" fill="currentColor" class="w-full h-full">
        ...
        <!-- Android -->
        <svg v-else-if="isAndroid" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 18" fill="currentColor" class="w-full h-full">
...
const isMac = computed(() => slug.value.includes('mac') || slug.value.includes('apple') || slug.value.includes('osx'));
const isLinux = computed(() => slug.value.includes('linux') || slug.value.includes('ubuntu') || slug.value.includes('steamos'));
const isAndroid = computed(() => slug.value.includes('android'));
const isWeb = computed(() => slug.value.includes('web'));
const isPC = computed(() => slug.value.includes('pc') || slug.value.includes('steam') || slug.value.includes('windows'));
...
const classes = computed(() => {
    let base = `flex items-center justify-center platforms__platform platforms__platform_medium platforms__platform_${platformClassSlug.value}`;
    return `${base} ${props.className}`;
});

const platformClassSlug = computed(() => {
    if (isPC.value) return 'pc';
    if (isPlayStation.value) return 'playstation';
    if (isXbox.value) return 'xbox';
    if (isNintendo.value) return 'nintendo';
    if (isMac.value) return 'mac';
    if (isLinux.value) return 'linux';
    if (isAndroid.value) return 'android';
    if (isWeb.value) return 'web';
    return slug.value;
});
</script>
